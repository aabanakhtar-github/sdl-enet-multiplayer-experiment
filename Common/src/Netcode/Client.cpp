...
void NetClient::connectTo(const std::string& server_addr, const std::uint16_t port, const float timeout) {
    if (client_ == nullptr) { 
        std::cerr << "Failed to connect to server! Client was nullptr." << std::endl; 
        return;
    }
    ...
    if (enet_host_service(client_, &event, static_cast<int>(timeout * 1000.f)) > 0
...
void NetClient::sendPacket(PacketData& packet, const int channel, const bool reliable) {
    ...
    ENetPacket* netpacket = packetDataToEnetPacket(packet, reliable ? ENET_PACKET_FLAG_RELIABLE : ENET_PACKET_FLAG_UNSEQUENCED); 
...
void NetClient::updateNetwork(const float block_time, const bool disconnection) {
    ...
    while (getConnected() &&
        enet_host_service(client_, &event, static_cast<std::uint32_t>(block_time * 1000.0f)) > 0) {
        switch (event.type) {
        ...
        case ENET_EVENT_TYPE_RECEIVE: {
            ...
            default:    
                if (!disconnection && parsed_packet.salt == (client_salt_ ^ server_salt_)) {
