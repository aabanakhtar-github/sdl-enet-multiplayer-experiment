...
void GraphicsSystem::update(ECS::Scene &scene, float delta) {
    ...
    // TODO: potentially add position component support
    ...
    drawGroupPhysicsEntities(scene, manager, physics_texture_groups.getEntities());

	game_window_.showBuffers();
}

void GraphicsSystem::drawGroupPhysicsEntities(ECS::Scene& scene, TextureManager &manager, const std::vector<ECS::EntityID> &IDs) {
   for (ECS::EntityID ID : IDs) {
       ...
       for (auto&[rect, texture] : component.textures) {
           ...
           game_window_.renderTexture();  // TODO: fill out
       }
   }
}

void GraphicsSystem::drawNormalEntities(ECS::Scene& scene, TextureManager &manager, const std::vector<ECS::EntityID> &IDs) {
    for (ECS::EntityID ID : IDs) {
        ...
        if (!texture.getValid()) {
            std::cerr << "Cannot render texture named " << texture_component.texture_name << "! SDL_Error: "
        ...
        if (position_component.cartesian) {
            ...
            destination = {transformed.x, transformed.y, static_cast<float>(src.w), static_cast<float>(src.h)};
        } else {
            destination = {position_component.position.x, position_component.position.y, static_cast<float>(src.w),
...
void GraphicsSystem::drawPhysicsEntities(ECS::Scene& scene, TextureManager &manager, const std::vector<ECS::EntityID> &IDs)  {
    for (ECS::EntityID ID : IDs) {
        auto& texture_component = scene.getComponent<TextureComponent>(ID);
        ...
        if (!texture.getValid()) {
            std::cerr << "Cannot render texture named " << texture_component.texture_name << "! SDL_Error: " << SDL_GetError() << std::endl;
            continue;
        }
