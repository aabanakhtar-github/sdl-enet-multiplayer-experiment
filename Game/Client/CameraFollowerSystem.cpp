...
void CameraFollowerSystem::update(ECS::Scene& scene, float delta) {
    ...
    auto& physics_component = scene.getComponent<PhysicsBodyComponent>(event_system->getThisClientID());


    graphics_system->camera.setOffset({ physics_component.AABB.x, physics_component.AABB.y });
}