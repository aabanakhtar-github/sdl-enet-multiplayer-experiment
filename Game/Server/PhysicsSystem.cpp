...
void PhysicsSystem::update(ECS::Scene &scene, float delta) {
    ...
    for (auto& ID : physics_bodies.getEntities()) {
        ...
        if (component.simulates_physics) component.velocity += gravity_ * delta;
...
void PhysicsSystem::handleCollisions(ECS::Scene& scene, float delta, ECS::EntityID ID, const std::vector<ECS::EntityID>& physics_bodies) {
    ...
    for (ECS::EntityID other : physics_bodies) {
        ...
        Vector2 position = graphics_system->camera.transform({ component.AABB.x, component.AABB.y });
        FRect rect = { position.x, position.y, component.AABB.w, component.AABB.h };
        Vector2 other_position = graphics_system->camera.transform({ other_component.AABB.x, other_component.AABB.y });
        FRect other_rect = { other_position.x, other_position.y, other_component.AABB.w, other_component.AABB.h };
